...
export default function StudentsPage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from('students')
      .select('id,name,phone,password_hash,progress_surah,progress_ayah,progress_page')
      .order('name');
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingStudent) {
        ...
        // Add new student
        const hash = await bcrypt.hash(formData.password, 10);
        const { error } = await supabase.from('students').insert({ ...studentData, password_hash: hash });
        if (error) throw error;
      }
      await load();
      closeModal();
